#### 3.1 fabric-sde-node：链码安装和实例化

1、在kevin-marbles/utils目录下，新建fc_wrangler/parts目录并进入

```
mkdir -p fc_wrangler/parts && cd fc_wrangler/parts
```

2、在kevin-marbles/utils/fc_wrangler/parts目录下，新建common.js、deploy_cc.js、enrollment.js共3个文件

**①common.js**

在utils/fc_wrangler/parts目录下，新建common.js文件，作为fabric客户端助手公共库，对信息进行处理
...
在utils/fc_wrangler/parts目录下，新建enrollment.js文件，其使用Hyperledger Fabric Client对用户的KVS进行管理，支持创建sdk实例，根据指定的键值对存储路径获取通过CA颁发的注册证书
...
在utils/fc_wrangler/parts目录下，新建deploy_cc.js文件，该文件作为链码部署库，支持安装、实例化和升级链码
...
在utils/fc_wrangler目录下，新建index.js文件，该文件支持调用链码，实现对链码的安装和实例化，也支持对分类账本的状态数据进行操作
